...
-- 拓展 v 的范围
-- 普通模式下双击v，选中当前单词
vim.keymap.set({ "n" }, "vv", function() vim.cmd("norm viw") end, { noremap = true },
  { desc = "Select the current word." })

-------------------------------------------------------------------------------------------
--buffer 和 窗口（win）
-- 切换buffer
vim.keymap.set({ "i", "n" }, "<C-S-h>", "<cmd>bprevious<cr>", { desc = "Prev Buffer" }) -- 将<C-S-h>映射为转到前一个buffer
vim.keymap.set({ "i", "n" }, "<C-S-l>", "<cmd>bnext<cr>", { desc = "Next Buffer" })     -- 将<C-S-l>映射为转到下一个buffer
